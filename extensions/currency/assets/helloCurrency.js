...
const currencySpan = document.querySelector('.curriency_metafiled_span');
console.log('span', currencySpan.textContent);
...
if (currencySpan) {
    try {
        ...
        // Populate the currency list dynamically
...
document.getElementById('custom-button').textContent = `${currencyCode}`;
...
// Event listener for currency selection (now dynamically added)
document.getElementById('currency_content_list').addEventListener('click', function (event) {
    if (event.target.tagName === 'LI') {
        const selectedCurrency = event.target.getAttribute('value');
        document.getElementById('custom-button').textContent = `${selectedCurrency}`;
        var currencyList = document.getElementById('currency-list');
        currencyList.style.display = 'none';
        localStorage.setItem('selectedCurrency', selectedCurrency); // Save currency in localStorage

        console.log('currencyCode this is the base currency', currencyCode);
...
// Restore selected currency on page load
